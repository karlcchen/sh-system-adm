...
function usage() {
    ...
    echo "  -x : (\$DEBUG)     debug"
    echo "  -6 : (\$IP6)       IPv6"
    echo "  -q : (\$VLAN_ID)   vlan id"
    echo ""
}

##  --- Parse command line arguments / parameters ---
## echo "Commandline options:"
while getopts "s:i:d:m:f:t:c:n:b:q:vxh6" option; do
    case $option in
        i) # interface
          export DEV=$OPTARG
	  ...
	  info "SKB bursting: BURST=$BURST"
          ;;
        q) # VLAN_ID
	  export VLAN_ID=$OPTARG
	  info "Vlan id set to: VLAN_ID=$VLAN_ID"
          ;;
        v)
          export VERBOSE=yes
...
if [ ! -d /proc/net/pktgen ]; then
    echo "INFO: Loading kernel module: pktgen..."
    modprobe pktgen
    if [ $? -ne 0 ] ; then 
	echo "ERROR: modprobe pktgen failed!"
	exit 1
    fi 
    sleep 3
    if [ ! -d /proc/net/pktgen ]; then
	echo "ERROR: /proc/net/pktgen not found after install 'pktgen' kernel modules!"
	exit 2
    fi 
else
    echo "INFO: /proc/net/pktgen found, use kernel pktgen for traffic generation..."
fi
